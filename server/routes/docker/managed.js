...
// Check if a managed container is actually running PZ successfully
// (not crash-looping due to missing libs, bad start script, etc).
// Polls container status and recent logs to diagnose common issues.
router.get("/servers/:id/health", async (req, res) => {
  try {
    ...
    // Detect crash-loop: container not running after startup
    ...
    // Check restart count — high restarts suggest crash-looping
    ...
    // Fetch recent logs to detect common errors
    if (containerId) {
      try {
        const logResult = await dockerClient.getContainerLogs(containerId, 50);
        if (logResult.success && logResult.lines) {
          const logs = logResult.lines.join("\n");

          if (/lib32gcc|libstdc\+\+|cannot open shared object/i.test(logs)) {
            health.issues.push({
              severity: "critical",
          ...
          if (/java.*OutOfMemoryError|Cannot allocate memory/i.test(logs)) {
            health.issues.push({
              severity: "critical",
          ...
          if (/ENOSPC|No space left on device/i.test(logs)) {
            health.issues.push({
              severity: "critical",
