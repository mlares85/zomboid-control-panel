...
Default container image is `debian:bookworm-slim` (PZ bundles its own JRE 25 at `jre64/` — no image-provided Java needed; using a JRE image puts a conflicting Java on PATH). The inline entrypoint handles first-boot setup: installs `lib32gcc-s1`, `libstdc++6:i386`, `ca-certificates`, `libssl3`, `unzip`; extracts `libsqlitejdbc.so` from `projectzomboid.jar` into `linux64/` (PZ's JNI-launched JVM can't extract it at runtime); sets `LD_LIBRARY_PATH` to include `jre64/lib/server` (JRE 25 moved this from the `lib/amd64` path PZ's `start-server.sh` still references); symlinks `/root/Zomboid` to the data volume for persistence; and pre-creates the server INI with `RCONEnabled=true` + password from env vars. The `--` separator between the entrypoint script and Docker Cmd args is critical — without it, `bash -c` consumes the first Cmd arg as `$0` and `-servername` is lost.
