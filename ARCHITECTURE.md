...
- **`NativeSteamCmdInstaller`** — wraps the SteamCMD child-process logic with concurrent-op guard, LD_LIBRARY_PATH setup, Linux auto-download fallback, and Steam depot access denied detection. Auto-retries up to 3 times on incomplete downloads (SteamCMD state 0x202/0x602) since SteamCMD resumes where it left off. Post-download verification checks for PZ server signatures (`ProjectZomboid64.exe`, `start-server.bat`) and the bundled JRE (`jre64/`) — catches "exit 0 but nothing downloaded" edge cases. `POST /install` and `POST /steam-update` delegate to it (strangler cutover complete); routes still own input validation, HTTP response shape, and post-install orchestration (`completeSuccessfulInstall`, event logging). The `/install` route runs a preflight disk space check (3.5 GB minimum) via `statDisk()` before spawning SteamCMD.
...
Both report progress via an `onProgress(event, data)` callback (not Socket.IO directly) so they're testable without a live server. Routes bridge the callback to Socket.IO event names (`install:log`, `steam:log`, etc.). `detectInstall.js` scans platform-specific paths for SteamCMD binaries and existing PZ server installs, exposed via `GET /api/server/setup/detect`. On Windows, `discoverSteamLibraryPaths()` parses Steam's `libraryfolders.vdf` to find PZ installs in Steam library folders on any drive (not just C:\). `installer/index.js` provides `getNativeInstaller()` factory wired to the real SteamCMD helpers.

`writeStartupScriptFiles()` (startupScripts.js) is platform-aware: only writes `.bat` on Windows, `.sh` on Linux — no cross-platform scripts that confuse users. The `/steamcmd/detect` endpoint returns a `suggestedPath` when SteamCMD isn't found (Documents/SteamCMD on Windows, ~/steamcmd on Linux) so the client can pre-fill the path field for first-time users.
...
## Docker-managed server backups

Docker-managed servers have `zomboidDataPath: null` — the panel can't read the container's filesystem. `createDockerBackup` (dockerBackup.js) streams save files out of the container via the Docker archive API (`GET /containers/{id}/archive?path=...`), which returns a raw tar. The stream is gzipped on the fly via `DockerClient.getArchive(id, path, destPath, { compress: true })`, producing a `.tar.gz` without a second full-file pass. Always full backups (no incremental — can't scan directory contents inside the container cheaply). `backupCreateHandler.js` detects `docker-managed` provider and routes through `createDockerBackup` instead of the filesystem-based orchestrator. Server snapshot is best-effort: INI/sandbox reads return empty since config files are inside the container, but server identity, provider, template, and mods are captured from the DB. Restore for Docker-managed servers is not yet implemented.

## Container log streaming

`DockerClient.streamContainerLogs(id, onLine, opts)` opens a persistent `GET /containers/{id}/logs?follow=true` connection and parses Docker's 8-byte multiplexed frame headers incrementally as data arrives. Returns a cancel function. Auto-reconnects on disconnect (same pattern as `watchEvents`). `ContainerLogStreamer` service manages the stream for the active server's container — subscriber-driven via Socket.IO rooms: starts when a client emits `subscribe:container-logs`, stops when the `container:logs` room empties. Emits `container:log` events (`{ line, stream: 'stdout' | 'stderr' }`). Reattaches on active server change (called from the activation route). Frontend `ContainerLogs` component fetches an initial 200-line tail via REST, then streams live updates via Socket.IO. Collapsible, terminal-styled, auto-scrolling with a 500-line buffer cap.
...
- `dockerContainerFactory.js` — builds Docker API container specs with volume mounts (`base→/opt/pz-server`, `srv→/root/Zomboid`), port bindings (game UDP + RCON TCP), management labels (`zomboid-panel.managed`, `zomboid-panel.server-id`), and the inline entrypoint. `createManagedServer(config)` orchestrates the full flow: ensure base volume → create server volume → pull image if needed → create container. `createServerVolume` is idempotent (checks if volume exists before creating). `findAvailablePorts(existingServers)` auto-assigns non-conflicting game/RCON port pairs.

Default container image is `debian:bookworm-slim` (PZ bundles its own JRE 25 at `jre64/` — no image-provided Java needed; using a JRE image puts a conflicting Java on PATH). The inline entrypoint handles first-boot setup: installs `lib32gcc-s1`, `libstdc++6:i386`, `ca-certificates`, `libssl3`, `unzip`; extracts `libsqlitejdbc.so` from `projectzomboid.jar` into `linux64/` (PZ's JNI-launched JVM can't extract it at runtime); sets `LD_LIBRARY_PATH` to include `jre64/lib/server` (JRE 25 moved this from the `lib/amd64` path PZ's `start-server.sh` still references); and pre-seeds the server INI with `RCONPort` + `RCONPassword` from env vars. **Important:** PZ has no `RCONEnabled` key — RCON is enabled implicitly when `RCONPassword` is non-empty. PZ preserves existing key values when it inflates a stub INI into its full ~420-line default on first boot, so pre-seeding works reliably. The `--` separator between the entrypoint script and Docker Cmd args is critical — without it, `bash -c` consumes the first Cmd arg as `$0` and `-servername` is lost.

Containers mount base files at `/opt/pz-server` (writable — PZ writes temp files and native lib extractions into its install dir) and the per-server data volume at `/root/Zomboid` (where PZ running as root writes saves/configs regardless of HOME). A `zomboid-panel-net` bridge network is auto-created so RCON traffic between the panel and managed servers stays internal. The panel container must be on this network (configured in compose via `networks: zomboid-panel-net: external: true`). Docker DNS resolves the managed container name for RCON (`rconHost` is the container name, not `127.0.0.1`). `zomboidDataPath` is set to `null` for managed servers since the panel can't access the container's filesystem — bridge filesystem IPC doesn't work across containers, and the dashboard verdict skips the bridge check for `docker-managed` providers. Containers get `RestartPolicy: unless-stopped`. Creation has rollback: if `startContainer` fails after the container and DB record are created, both are cleaned up. The preflight check uses the container-visible path (not the resolved host path) to validate `start-server.sh` exists. `DockerClient.watchEvents()` streams container lifecycle events via the Docker API as `docker:event` over Socket.IO. The `/start` route returns success (not an error) when the container is already running.
...
## World map tile proxy

`mapProxy.js` is a single consolidated file (ported from upstream) that proxies PZ map tiles from two external hosts: `pzmap.org` (build-list API at `/api/builds/default`) and `tiles.pzmap.org` (tile bytes, no `/maps/` prefix). The old `map.projectzomboid.com` host is dead. Cloudflare bot-management blocks Node's TLS stack for metadata (JSON/XML/DZI descriptors) but not for tile bytes, so metadata fetches use `curl` via `fetchViaCurl()` while tile serving uses plain `fetch`. Build resolution caches the result for 24h with 5-minute retry on failure, and coalesces concurrent cold-cache resolves via a shared promise (`_b42ResolvePromise`). The `MapVersionChecker` service was removed — version detection is now internal to the proxy with its own TTL caching.

Tile caching is three-tier: in-memory LRU (500 entries) → persistent disk cache (`data/map-tiles-cache/`) → upstream fetch with circuit breaker (opens after 8 consecutive failures, 30s cooldown). PZ map tiles are immutable per build, so disk cache entries never expire. `discoverRenderedMaxLevel()` binary-searches the `[maxLevel-6, maxLevel]` range for the deepest zoom level with real tiles, preventing the client from requesting tiles that 404 (which caused solid black terrain — GH#109). WorldMap.tsx clamps tile requests to `renderedMaxLevel` and uses `resolveFallbackTile()` (worldMapTileFallback.ts) to draw a coarser cached tile's sub-rectangle when the exact tile is missing. `inFlightGate` (client/src/lib/inFlightGate.ts) prevents overlapping player/overlay bridge fetches.
...
The entrypoint installs 32-bit compat libs, extracts SQLite native libs, configures RCON, and fixes `LD_LIBRARY_PATH` for JRE 25 — see the Docker-managed containers section above for details. A `/tmp` tmpfs mount is included in HostConfig. `GET /api/docker/managed/servers/:id/health` inspects container state, restart count, and recent logs for common failures (missing libs, OOM, disk full), returning structured issues with severity and remediation hints.
